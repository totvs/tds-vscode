import { StatusBar, WebElement, Workbench, Notification } from "vscode-extension-tester";
...
export class WorkbenchPageObject {
	...
	async isConnected(LOCALHOST_NAME: string, LOCALHOST_ENVIRONMENT: string): Promise<boolean> {
		return (await this.statusBar.statusBarWithText(`${LOCALHOST_NAME} / ${LOCALHOST_ENVIRONMENT}`, 10000)) != null
	}

	async isNeedSelectServer(): Promise<boolean> {
		return (await this.statusBar.statusBarWithText(/Select server\/environment/)) != null
	}

	async isLoggedIn(): Promise<boolean> {
		return (await this.statusBar.statusBarWithText(/Permissions: Logged in/)) != null
	}

	async isNotLoggedIn(): Promise<boolean> {
		return (await this.statusBar.statusBarWithText(/Permissions: NOT logged in/)) != null
	}

	async waitConnection(wait: number = 30000): Promise<void> {
		if ((await this.statusBar.statusBarWithText(/Authenticating user.*/), wait) == null) {
			throw new Error(`Connection process timeout (${wait})ms`);
		}
	}

	async waitReconnection(wait: number = 30000): Promise<void> {
		if ((await this.statusBar.statusBarWithText(/.*Reconnecting to the server*./), wait) == null) {
			throw new Error(`Connection process timeout (${wait})ms`);
		}
	}

	async waitCheckIntegrity(wait: number = 30000): Promise<void> {
		if ((await this.statusBar.statusBarWithText(/Checking RPO integrity/), wait) == null) {
			throw new Error(`Check Integrity process timeout (${wait})ms`);
		}
	}

	async waitNotification(containText: string, dismiss: boolean = true): Promise<Notification> {
		const notification: Notification = await this.notification.waitNotification(containText, dismiss)
		return notification;
	}
