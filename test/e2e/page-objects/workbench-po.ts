...
export class WorkbenchPageObject {
  ...
  async isLoggedIn(): Promise<boolean> {
    return (
      (await this.statusBar.statusBarWithText(/Permissions: Logged in/)) != null
    );
  }
  ...
  async waitConnection(wait: number = 30000): Promise<void> {
    if (
      (await this.statusBar.statusBarWithText(/Authenticating user.*/), wait) ==
      null
    ) {
  ...
  async waitRevalidate(wait: number = 30000): Promise<void> {
    if (
      (await this.statusBar.statusBarWithText(/Revalidating RPO/),
      wait) == null
    ) {
  ...
  async waitNotification(
    ...
    const notification: Notification = await this.notification.waitNotification(
      containText,
      dismiss
    );
    return notification;
