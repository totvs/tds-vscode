...
class ServerItemProvider implements vscode.TreeDataProvider<ServerItem | EnvSection> {

	isConnected(server: ServerItem) {
		return this._connectedServerItem !== undefined &&
	...
	private _connectedServerItem: ServerItem | undefined = null;
	...
	getTreeItem(element: ServerItem | EnvSection): vscode.TreeItem {
		if (element instanceof ServerItem) {
			let iconPath = {
				light: path.join(__filename, '..', '..', 'resources', 'light', this._connectedServerItem !== undefined && element.id === this._connectedServerItem.id ? 'server.connected.svg' : 'server.svg'),
				dark: path.join(__filename, '..', '..', 'resources', 'dark', this._connectedServerItem !== undefined && element.id === this._connectedServerItem.id ? 'server.connected.svg' : 'server.svg')
			};
		...
		if (element) {
			if (element.environments) {
				...
				if (listOfEnvironments.size > 0) {
					this.localServerItems[element.id].environments = listOfEnvironments.map(env => new EnvSection(env, element, vscode.TreeItemCollapsibleState.None, {
						command: 'totvs-developer-studio.environmentSelection',
						title: '',
						arguments: [env]
					}));
					this.localServerItems[element.id].collapsibleState = vscode.TreeItemCollapsibleState.Expanded;
					//Workaround: Bug que nao muda visualmente o collapsibleState se o label permanecer intalterado
					this.localServerItems[element.id].label = this.localServerItems[element.id].label.endsWith(' ') ? this.localServerItems[element.id].label.trim() : this.localServerItems[element.id].label + ' ';
...
export class ServerItem extends vscode.TreeItem {
	...
	iconPath = {
		light: path.join(__filename, '..', '..', 'resources', 'light', this.isConnected ? 'server.connected.svg' : 'server.svg'),
		dark: path.join(__filename, '..', '..', 'resources', 'dark', this.isConnected ? 'server.connected.svg' : 'server.svg')
	};
...
export class EnvSection extends vscode.TreeItem {
	...
	iconPath = {
		light: path.join(__filename, '..', '..', 'resources', 'light', this.serverItemParent !== undefined && this.serverItemParent.id === this.serverItemParent.id && this.serverItemParent.currentEnvironment === this.label ? 'environment.connected.svg' : 'environment.svg'),
		dark: path.join(__filename, '..', '..', 'resources', 'dark', this.serverItemParent !== undefined && this.serverItemParent.id === this.serverItemParent.id && this.serverItemParent.currentEnvironment === this.label ? 'environment.connected.svg' : 'environment.svg')
	};
