							...
							if (message.serverName && message.port && message.address) {
								...
								if (serverId !== undefined) {
									languageClient.sendRequest('$totvsserver/validation', {
										...
										Utils.updateBuildVersion(serverId, validInfoNode.buildVersion, validInfoNode.secure == 1);
										return;
			...
			if (ix >= 0) {
				...
				if (serverItem.buildVersion) {
					...
					languageClient.sendRequest('$totvsserver/validation', {
						...
						const updated = Utils.updateBuildVersion(serverItem.id, validInfoNode.buildVersion, validInfoNode.secure == 1);
...
function sendConnectRequest(serverItem: ServerItem, environment: string, _connType: ConnTypeIds) {
...
export function reconnectServer(reconnectionInfo, environment: string, connType: ConnTypeIds): boolean {
		...
		if (servers.configurations) {
			servers.configurations.forEach(element => {
				if (element.id === servers.lastConnectedServer.id) {
					...
					sendReconnectRequest(serverItem, servers.lastConnectedServer.token, servers.lastConnectedServer.environment, ConnTypeIds.CONNT_DEBUGGER);
				}
			});
		}
	}
...
function sendReconnectRequest(serverItem: ServerItem, connectionToken: string, environment: string, _connType: ConnTypeIds) {
	languageClient.sendRequest('$totvsserver/reconnect', {
		...
		if (token) {
			...
			if (token !== connectionToken) {
				Utils.updateSavedToken(serverItem.id, environment, token);
			}
			Utils.saveSelectServer(serverItem.id, token, serverItem.name, environment, user);
			if (treeDataProvider !== undefined) {
				connectedServerItem = serverItem;
